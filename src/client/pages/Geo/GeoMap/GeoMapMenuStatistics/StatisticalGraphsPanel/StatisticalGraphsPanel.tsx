...
const StatisticalGraphsPanel: React.FC<Props> = (props: Props) => {
  const { data, fra1ALandArea, fra1aForestArea, countryIso, year } = props
  const chartTitle = `Forest extent estimations per product and reported on ${year} (1a)`
  ...
  labels.push(`Reported to FRA ${year}`) // Label for fra1aForestArea
  ...
  areas.push(fra1aForestArea * 1000) // Convert to Ha. Instead of Thousands of Ha.
  ...
  const fra1aForestAreaPercentage = (fra1aForestArea * 1000 * 100) / (fra1ALandArea * 1000)
  percentages.push(Number(fra1aForestAreaPercentage.toFixed(2))) // Percentage for fra1aForestArea.
  ...
  backgroundColors.push('#000000') // Black color for the fra1aForestArea bar.
